...
const IfoCardTokens: React.FC<React.PropsWithChildren<IfoCardTokensProps>> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough WCORE to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your WCORE.',
    ),
    { placement: 'bottom' },
  )
          ...
          <Box>
            <MessageText display="inline">
              {publicIfoData.status === 'finished'
                ? t('Stay Tuned for our next IFO‘s!')
                : t('Commit WCORE in Accordance to the iSHDW Available')}
            </MessageText>{' '}
          </Box>
        </Message>
      )
    ...
    const ifov31Msg =
      ifo.version >= 3.1 && poolId === PoolIds.poolBasic && criterias?.length > 0 ? (
        <Box mt="16px">
          ...
          criterias={criterias}
          isWhitelisted={publicPoolCharacteristics?.isWhitelisted}
          {isEligible && (
    ...
    if (
      ...
      message = (
        <Message my="24px" p="8px" variant="danger">
          <Box>
            <MessageText display="inline">{t('You don’t have any iSHDW available for IFO public sale.')}</MessageText>{' '}
          ...
          {ifov31Msg || (
            <>
              <Text textAlign="center" fontSize="14px">
                {t('To participate in the next IFO, lock some SHDW in the fixed-term staking SHDW pool!')}
              </Text>

...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version >= 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of SHDW up to the maximum commit limit, which is published along with the IFO voting proposal/information.',
        )
      : t(
          'For the basic sale, Max WCORE entry is capped by minimum between your average WCORE balance in the iSHDW, or the pool’s hard cap. To increase the max entry, Stake more SHDW into the iSHDW',
        )

  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      <Box>
        <Text display="inline">{t('For the public sale, Max SHDW entry is capped by')} </Text>
        <Text bold display="inline">
          {t('the number of iSHDW.')}{' '}
        </Text>
        <Text display="inline">
          {t('Lock more SHDW for longer durations to increase the maximum number of WCORE you can commit to the sale.')}
        </Text>
      </Box>
    ) : (
      t(
        'For the unlimited sale, Max WCORE entry is capped by your average WCORE balance in the iShdw. To increase the max entry, Stake more SHDW into the iShdw',
      )
    )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. WCORE entry') : t('Max. token entry')
      ...
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && (
            <FooterEntry label={t('WCORE to burn:')} value={ifo[poolId].cakeToBurn} />
          )}
          <FooterEntry

  ...
  return (
    <>
      <Box>
        ...
        <LightGreyCard>
          <BalanceRow title={t('Shdw to be locked')} value={lockedAmount} newValue={newLockedAmount} decimals={2} />
          <BalanceRow title="iShdw" decimals={2} value={formattediCake} newValue={newFormattediCake} />
          <BalanceRow
            title="apy"
            unit="%"
      ...
      {showLockWarning && (
        <Box mt="16px" maxWidth="370px">
          <Message variant="warning">
            <MessageText>
              {t('You will be able to withdraw the staked SHDW and profit only when the staking position is unlocked')}
            </MessageText>
          </Message>
        </Box>
      )}

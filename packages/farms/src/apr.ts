...
export const getFarmCakeRewardApr = (farm: FarmWithPrices, cakePriceBusd: FixedNumber, regularShdwPerBlock: number) => {
  ...
  const yearlyCakeRewardAllocation = poolWeight
    ? poolWeight.mulUnsafe(FixedNumber.from(BLOCKS_PER_YEAR).mulUnsafe(FixedNumber.from(String(regularShdwPerBlock))))
    : FIXED_ZERO
